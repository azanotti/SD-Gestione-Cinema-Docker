...
function getFilmDetails(filmId) {
    fetch(`http://88.99.80.223/film/${filmId}`)
        .then(response => response.json())
        .then(film => {
...
function bookSeats() {
    ...
    if (selectedSeats.length > 0) {
        ...
        fetch('http://88.99.80.223/booking', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function loadDateOptions() {
    ...
    const projectionListUrl = `http://88.99.80.223/proiezione/${selectedMovieId}`;
